...
class GraphColoringEnv:
    ...
    def get_valid_actions(self):
        """Returns a mask indicating valid actions. This looks at the neighbors to see if the graph is valid"""
        mask = torch.zeros((self.num_nodes, self.num_nodes), dtype=torch.bool, device=device)
    ...
    def step(self, node, color):
        """Applies the action (coloring a node/or an edge)."""
        self.state[node] = color
        if all(v is not None for v in self.state.values()):
            self.done = True

    def get_reward(self):
        """Returns the reward based on the number of colors used. Here is """
...
class GFlowNet(nn.Module):
    def __init__(self, num_nodes):
        ...
        self.policy_net = nn.Sequential(
            nn.Linear(num_nodes * 2, 1024),
            nn.ReLU(),
            nn.Linear(1024, num_nodes * num_nodes)  # Flattened action space
        )
...
# Define the loss function for Trajectory Balance
def trajectory_balance_loss(model, trajectory, reward):
    log_probs = []
    for state_tensor, (node, color) in trajectory:
        logits = model(state_tensor.unsqueeze(0).to(device))
...
graph = nx.erdos_renyi_graph(15, 0.6, seed=42)
env = GraphColoringEnv(graph)
num_nodes = len(graph.nodes)

# Initialize the model and optimizer
model = GFlowNet(num_nodes).to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=1e-5)

# Training loop
for epoch in range(1000):
    env = GraphColoringEnv(graph)  # Reset environment
    state_tensor = torch.zeros(num_nodes * 2, device=device)  # Example state representation
    traj = []
    ...
    while not env.done:
        ...
        mask = mask.unsqueeze(0).expand_as(logits)  # Match batch dimension
...
def count_unique_colors(state_tensor):
    if isinstance(state_tensor, torch.Tensor):  # If it's a torch tensor
        colors = state_tensor.tolist()  # Convert tensor to a list (if it's a 1D tensor)
    else:
        raise ValueError("Unsupported type for state_tensor")
    
    # Return the number of unique colors
    return len(set(colors))

# Sort the traj list by the number of unique colors in ascending order
sorted_traj = sorted(traj, key=lambda x: count_unique_colors(x[0]))
print(sorted_traj)
# Print the best 10 performing states with the least unique colors
for i in range(min(10, len(sorted_traj))):
    state_tensor, (node, color) = sorted_traj[i]
    unique_colors = count_unique_colors(state_tensor)  # Get the number of unique colors
    print(f"State {i+1}: Unique Colors: {unique_colors}")
