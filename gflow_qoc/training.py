...
def GIN_TB_train(num_colors, num_nodes, FEATURE_KEYS, target_expr, target_state, n_episodes, learning_rate, decay_rate, seed, update_freq, max_edges, n_hid_units, n_emb):
    set_seed(seed)

    # Instantiate model and optimizer
    model = GIN_TBModel(num_nodes, n_hid_units, FEATURE_KEYS)
    #model = GINE_TBModel(num_nodes, n_emb, n_hid_units, FEATURE_KEYS)
    opt = torch.optim.Adam(model.parameters(), learning_rate)
    scheduler = torch.optim.lr_scheduler.ExponentialLR(opt, gamma=decay_rate)
